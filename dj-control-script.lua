...
-- Note: MessagingService only works in Live Server, not in Studio unless using Team Test
print("DJ Control Script starting - MessagingService requires Live Server for cross-server communication")
...
local function fadeDJModel(fadeIn)
    ...
    for _, descendant in pairs(djModel:GetDescendants()) do
        if descendant:IsA("BasePart") then
            -- Reset transparency before tweening to ensure proper fade effect
            if fadeIn then
                descendant.Transparency = 1
            end
